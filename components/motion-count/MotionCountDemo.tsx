...
export function MotionCountDemo() {
    ...
    switch (tracking.trackingState) {
      ...
      case 'interrupted':
        return {
          ...
          body: tracking.errorMessage ?? '장치 분리, 브라우저 권한 변경, 다른 앱의 점유 여부를 확인한 뒤 다시 연결해주세요.',
          action: '다시 연결',
        }
      case 'unsupported':
        return {
      ...
      case 'error':
        return {
          ...
          body: tracking.errorMessage ?? '네트워크 상태 또는 브라우저 권한을 확인한 뒤 다시 시도해주세요.',
          action: '다시 시도',
        }
      case 'ready':
      default:
  ...
  return (
    <main className="app-shell">
      ...
      <HandOverlay hands={tracking.hands} rawDetectionCount={tracking.rawDetectionCount} />
      ...
      <section className="scene-overlay">
        <header className="title-lockup">
          ...
          <p className="title-lockup__body">지금 감지된 숫자는 {tracking.fingerCount}입니다.</p>
        </header>
          ...
          <div className="launch-panel__content">
            ...
            <div className="launch-panel__actions">
              ...
              <p className="launch-panel__hint">
                최신 데스크톱 Chrome 권장 · 카메라 화면은 숨기고 손 인식 포인트만 표시합니다.
              </p>
            </div>
          </div>
        </section>
